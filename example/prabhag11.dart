...
void main() {
  ...
  var in1 = Point(18.507305, 73.806131);
  ...
  print(location_eleven.isPointInside(in1));

  for (var i in checkPunches) {
    out.add(location_eleven.isPointInside(i));
    x.add(i.x);
    y.add(i.y);
  }
  print(out);
  var out_l = out.length;
...
//    x[i] = e[i].x;
//    y[i] = e[i].y;
    var Temp = [x[i], y[i], out[i]];
    fe.add(Temp);
  }
//  e.forEach(f)
//  fe = [e, out];
  var csv = const ListToCsvConverter().convert(fe);
//  var fil = File('out1.csv').createSync(recursive: true);
  var file = File('out1.csv');
//  var sink = file.openSync();
